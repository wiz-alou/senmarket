...
export default function DashboardPage() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // Appel parallel de toutes les APIs avec gestion d'erreurs individuelles
      const [dashboardRes, listingsRes, contactsRes, paymentsRes] = await Promise.allSettled([
      ...
      // Traitement des réponses avec gestion d'erreurs
      if (dashboardRes.status === 'fulfilled' && dashboardRes.value.ok) {
        const dashboardData = await dashboardRes.value.json()
        console.log('✅ Stats dashboard reçues:', dashboardData.stats)
        setStats(dashboardData.stats)
      } else {
        console.error('❌ Erreur dashboard API:', dashboardRes)
          ...
          >
            <Tabs defaultValue="listings" className="space-y-6">
                ...
                <Card className="bg-white rounded-2xl shadow-xl border border-slate-200">
                  <CardHeader className="pb-4">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                      <CardTitle className="text-2xl font-bold text-slate-900">Mes annonces</CardTitle>
              ...
              <TabsContent value="messages" className="space-y-6">
                <Card className="bg-white rounded-2xl shadow-xl border border-slate-200">
                  <CardHeader>
                    <CardTitle className="text-2xl font-bold text-slate-900">Messages reçus</CardTitle>
                  </CardHeader>
                  <CardContent>
                    {contacts.length === 0 ? (
                      <div className="text-center py-12">
