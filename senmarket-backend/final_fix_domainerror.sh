#!/bin/bash
# FIX FINAL - DOMAINERROR USAGE

echo "🔧 FIX FINAL - DOMAINERROR USAGE"
echo "=================================="

# Je vois que DomainError est défini comme : type DomainError string
# Donc on doit utiliser les constantes définies dans errors.go
# Ou créer des erreurs personnalisées

# Corriger les erreurs dans user_repository.go
sed -i 's/return nil, &entities.DomainError{[^}]*}/return nil, entities.ErrUserNotVerified/g' internal/infrastructure/persistence/postgres/user_repository.go

# Corriger les erreurs dans listing_repository.go  
sed -i 's/return nil, &entities.DomainError{[^}]*}/return nil, entities.ErrUnauthorizedListing/g' internal/infrastructure/persistence/postgres/listing_repository.go

# OU créer des erreurs avec fmt.Errorf pour plus de flexibilité
...
# Corriger listing_repository.go avec fmt.Errorf
...
echo "✅ Repositories corrigés avec fmt.Errorf"
...
if go build ./internal/infrastructure/persistence/postgres/...; then
    ...
    if go build ./internal/infrastructure/...; then
        ...
        echo ""
        echo "🎉 PHASE 4A - POSTGRESQL REPOSITORIES - SUCCÈS TOTAL !"
        echo "====================================================="
        ...
        echo "✅ Gestion des erreurs avec fmt.Errorf"
        echo "✅ Types compatibles : IsActive, ExpiresAt, DeletedAt, Email"
        echo ""
        echo "🚀 PRÊT POUR PHASE 4B : Redis Cache Repository + Service Adapters"
    else
